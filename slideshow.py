...
# モデル設定（キー: 'anime' | 'photo'）
_ESRGAN_MODELS: dict[str, dict[str, Any]] = {
    'anime': {
        'url':       'https://github.com/xinntao/Real-ESRGAN/releases/download/v0.2.2.4/RealESRGAN_x4plus_anime_6B.pth',
        'path':      Path.home() / '.cache' / 'realesrgan' / 'RealESRGAN_x4plus_anime_6B.pth',
        'num_block': 6,
    },
    'photo': {
        'url':       'https://github.com/xinntao/Real-ESRGAN/releases/download/v0.1.0/RealESRGAN_x4plus.pth',
        'path':      Path.home() / '.cache' / 'realesrgan' / 'RealESRGAN_x4plus.pth',
        'num_block': 23,
    },
}
...
def _init_clip() -> tuple[Any, Any]:
    ...
    transformers.logging.set_verbosity_error()  # INFO/WARNING ログを抑制
...
def _detect_model_type(arr_bgr: np.ndarray, clip_model: Any, clip_processor: Any) -> str:
    ...
    # _CLIP_LABELS[0] = 'a photograph', [1] = 'an illustration'
...
def _init_esrgan(model_type: str) -> Any:
    """指定モデルを初期化して返す。モデルファイルがなければダウンロードする。"""
    warnings.filterwarnings('ignore', category=FutureWarning, module='realesrgan')
    _patch_torchvision()
    from basicsr.archs.rrdbnet_arch import RRDBNet
    from realesrgan import RealESRGANer

    cfg = _ESRGAN_MODELS[model_type]
    cfg['path'].parent.mkdir(parents=True, exist_ok=True)
    if not cfg['path'].exists():
        print(f'Real-ESRGANモデルをダウンロード中 ({model_type})...', flush=True)
        urllib.request.urlretrieve(cfg['url'], cfg['path'])
        print('ダウンロード完了', flush=True)

    model = RRDBNet(num_in_ch=3, num_out_ch=3, num_feat=64,
                    num_block=cfg['num_block'], num_grow_ch=32, scale=4)
    return RealESRGANer(
        scale=4,
...
class SlideShow:
    def __init__(self, folder: str):
        ...
        # ESRGANモデルキャッシュ（初回使用時にオンデマンドでロード）
        self._upsamplers: dict[str, Any] = {}
        # CLIPモデル（初回使用時にオンデマンドでロード）
    ...
    def _get_clip(self) -> tuple[Any, Any]:
        """CLIPモデルを返す。未ロードなら初期化してキャッシュする。"""
        if self._clip_model is None:
            self._clip_model, self._clip_processor = _init_clip()
        return self._clip_model, self._clip_processor

    def _get_upsampler(self, model_type: str) -> Any:
        """モデルを返す。未ロードなら初期化してキャッシュする。"""
        if model_type not in self._upsamplers:
            print(f'ESRGANモデルをロード中 ({model_type})...', flush=True)
            self._upsamplers[model_type] = _init_esrgan(model_type)
            print(f'ESRGANモデルのロード完了 ({model_type})', flush=True)
        return self._upsamplers[model_type]
    ...
    def _process(self, arr_bgr: np.ndarray) -> _Frame:
        ...
        if sc > 1.0 and not DEBUG_NO_ESRGAN:
            import torch
            model_type = _detect_model_type(arr_bgr, *self._get_clip())
            upsampler  = self._get_upsampler(model_type)
            with contextlib.redirect_stdout(io.StringIO()):
                arr_out, _ = upsampler.enhance(arr_bgr, outscale=4)
