...
def _init_esrgan(model_type: str) -> Any:
    ...
    return RealESRGANer(
        scale=4, model_path=str(cfg['path']), model=model,
        tile=256, tile_pad=10, pre_pad=0, half=True, gpu_id=0,
    )
...
class SlideShow:
    def __init__(self, folder: str):
        ...
        # モデルキャッシュ（初回使用時にオンデマンドでロード）
        self._clip:       tuple[Any, Any] | None = None
    ...
    def _process(self, arr_bgr: np.ndarray) -> _Frame:
        ...
        if sc > 1.0 and not DEBUG_NO_ESRGAN:
            import torch
            model_type = _classify_image(arr_bgr, self._get_clip())
